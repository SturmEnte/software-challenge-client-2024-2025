...
fn eval(game_state: &GameState, board: &Board) -> i32 {
    ...
    let mut eval = 0;
    if game_state.opponent_hare.position == 64 {eval -= 200000}
    eval += game_state.opponent_hare.salads as i32 * 5;
    eval -= game_state.your_hare.salads as i32 * 150;
    if game_state.your_hare.salads != 0 {eval += (game_state.your_hare.card_eat_salad as i32 * 50).clamp(0, 100)} 
